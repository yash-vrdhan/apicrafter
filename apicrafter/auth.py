...
class AuthHandler:
    ...
    def interactive_auth_setup() -> Optional[AuthConfig]:
        ...
        if auth_choice == AuthType.BEARER:
            ...
            key_name = questionary.text("API Key Name (e.g., 'X-API-Key', 'api_key'):").ask()
            ...
            questionary.print("OAuth2 support is coming in a future release!", style="fg:yellow")
            return AuthConfig(auth_type=AuthType.NONE)
...
def demo_auth_functionality():
    ...
    updated_headers, updated_params = AuthHandler.apply_auth(api_key_config, headers.copy(), params.copy())
    ...
    updated_headers, updated_params = AuthHandler.apply_auth(api_key_query_config, headers.copy(), params.copy())
