...
class RequestValidator:
    ...
    def validate_request(
        ...
        if method and method.upper() != endpoint.method:
            result.add_error("method", f"Expected {endpoint.method}, got {method.upper()}")
        ...
        if endpoint.query_params:
            query_result = self.validate_query_params(endpoint.query_params, query_params or {})
    ...
    def validate_headers(self, headers_schema: Dict[str, Any], headers: Dict[str, str]) -> ValidationResult:
        ...
        for header_name, header_def in headers_schema.items():
            if header_def.get('required', False):
                ...
                if not header_found:
                    result.add_error(f"headers.{header_name}", "Required header is missing")
    ...
    def validate_query_params(self, query_schema: Dict[str, Any], query_params: Dict[str, str]) -> ValidationResult:
        ...
        for param_name, param_def in query_schema.items():
            if param_def.get('required', False) and param_name not in query_params:
                result.add_error(f"query.{param_name}", "Required query parameter is missing")
    ...
    def validate_body(self, body_schema: Dict[str, Any], body: Optional[Union[Dict[str, Any], str]]) -> ValidationResult:
    ...
    def validate_object(self, obj: Any, schema: Dict[str, Any], field_path: str) -> ValidationResult:
        ...
        for required_field in required_fields:
            if required_field not in obj:
                result.add_error(f"{field_path}.{required_field}", "Required field is missing")
        ...
        for prop_name, prop_value in obj.items():
            if prop_name in properties:
                ...
                if additional_properties is False:
                    result.add_error(f"{field_path}.{prop_name}", "Additional property not allowed")
    ...
    def validate_array(self, arr: Any, schema: Dict[str, Any], field_path: str) -> ValidationResult:
        ...
        if min_items is not None and len(arr) < min_items:
            result.add_error(field_path, f"Array must have at least {min_items} items, got {len(arr)}")
        
        if max_items is not None and len(arr) > max_items:
            result.add_error(field_path, f"Array must have at most {max_items} items, got {len(arr)}")
    ...
    def validate_field_value(self, field_path: str, value: Any, schema: Dict[str, Any]) -> ValidationResult:
        ...
        if field_type == 'string' and not isinstance(value, str):
            result.add_error(field_path, f"Expected string, got {type(value).__name__}", value)
        elif field_type == 'integer' and not isinstance(value, int):
            result.add_error(field_path, f"Expected integer, got {type(value).__name__}", value)
        elif field_type == 'number' and not isinstance(value, (int, float)):
            result.add_error(field_path, f"Expected number, got {type(value).__name__}", value)
        elif field_type == 'boolean' and not isinstance(value, bool):
            result.add_error(field_path, f"Expected boolean, got {type(value).__name__}", value)
        elif field_type == 'array' and not isinstance(value, list):
            result.add_error(field_path, f"Expected array, got {type(value).__name__}", value)
        elif field_type == 'object' and not isinstance(value, dict):
            result.add_error(field_path, f"Expected object, got {type(value).__name__}", value)
        
        # Enum validation
        if enum_values and value not in enum_values:
            result.add_error(field_path, f"Value must be one of {enum_values}, got {value}", value)
        
        # String-specific validations
        if isinstance(value, str):
            if pattern and not re.match(pattern, value):
                result.add_error(field_path, f"Value does not match pattern: {pattern}", value)
            
            if min_length is not None and len(value) < min_length:
                result.add_error(field_path, f"String must be at least {min_length} characters, got {len(value)}", value)
            
            if max_length is not None and len(value) > max_length:
                result.add_error(field_path, f"String must be at most {max_length} characters, got {len(value)}", value)
        
        # Numeric validations
        if isinstance(value, (int, float)):
            if minimum is not None and value < minimum:
                result.add_error(field_path, f"Value must be at least {minimum}, got {value}", value)
            
            if maximum is not None and value > maximum:
                result.add_error(field_path, f"Value must be at most {maximum}, got {value}", value)
    ...
    def validate_path_parameters(self, schema_path: str, actual_path: str) -> ValidationResult:
        ...
        if len(schema_parts) != len(actual_parts):
            result.add_error("path", f"Path structure mismatch: expected {schema_path}, got {actual_path}")
            return result
        ...
        for i, (schema_part, actual_part) in enumerate(zip(schema_parts, actual_parts)):
            if schema_part.startswith('{') and schema_part.endswith('}'):
                ...
                if not actual_part:
                    ...
                    result.add_error(f"path.{param_name}", "Path parameter cannot be empty")
            elif schema_part != actual_part:
                result.add_error("path", f"Path segment mismatch at position {i}: expected '{schema_part}', got '{actual_part}'")
            ...
            for header_name, header_def in endpoint.headers.items():
                if not header_def.get('required', False) and header_def.get('default'):
                    if not headers or header_name not in headers:
                        result.add_suggestion(f"Consider adding header '{header_name}' with default value: {header_def['default']}")
            ...
            for param_name, param_def in endpoint.query_params.items():
                if not param_def.get('required', False) and param_def.get('default'):
                    if not query_params or param_name not in query_params:
                        result.add_suggestion(f"Consider adding query parameter '{param_name}' with default value: {param_def['default']}")
        
        # Suggest examples if available
        if endpoint.body_schema and not body:
            example = endpoint.body_schema.get('example')
            if example:
                result.add_suggestion(f"Example request body: {json.dumps(example, indent=2)}")
...
def demo_validator():
    ...
    valid_headers = {"Authorization": "Bearer token123", "Content-Type": "application/json"}
    ...
    invalid_body = {"username": "jo", "email": "invalid-email"}  # Short username, invalid email
