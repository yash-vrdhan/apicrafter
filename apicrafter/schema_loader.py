...
class SchemaLoader:
    ...
    def load_schema_from_file(self, file_path: str) -> Optional[APISchema]:
        ...
        try:
            ...
            base_url = schema_data.get('servers', [{}])[0].get('url', 'https://api.example.com')
    ...
    def get_endpoint_schema(self, schema: APISchema, method: str, path: str) -> Optional[SchemaEndpoint]:
        """
    ...
    def _parse_openapi_schema(self, schema_data: Dict[str, Any], base_url: str) -> APISchema:
        ...
        for path, path_data in paths.items():
            for method, method_data in path_data.items():
                if method.lower() in ['get', 'post', 'put', 'patch', 'delete', 'head', 'options']:
                    endpoint = self._parse_endpoint(path, method.upper(), method_data, api_schema.components)
                    api_schema.endpoints.append(endpoint)
        
        return api_schema
    
    def _parse_endpoint(self, path: str, method: str, method_data: Dict[str, Any], components: Dict[str, Any]) -> SchemaEndpoint:
    ...
    def _resolve_schema_ref(self, schema: Dict[str, Any], components: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'properties' in schema:
            ...
            for prop_name, prop_schema in schema['properties'].items():
                resolved_properties[prop_name] = self._resolve_schema_ref(prop_schema, components)
            schema = schema.copy()
    ...
    def list_endpoints(self, schema: APISchema, filter_method: Optional[str] = None) -> List[Tuple[str, str]]:
        """
