...
class AuthManager:
    ...
    def get_token(self, environment: str, token_type: str, request_name: Optional[str] = None) -> Optional[str]:
        """
        ...
        if token_info.expires_at:
            ...
            if time_until_expiry.total_seconds() <= 300:  # 5 minutes
                return self._handle_expired_token(token_info, environment, token_type, request_name)
    ...
    def _handle_expired_token(
        ...
        if action == "new":
            ...
            return self._refresh_token(token_info, environment, token_type, request_name)
        elif action == "skip":
            return None
        else:  # cancel
    ...
    def _prompt_for_new_token(
        ...
        if new_token:
            ...
            expires_in = None
            if questionary.confirm("Does this token have an expiration time?", default=False).ask():
                try:
                    hours = questionary.text("How many hours until expiration?", default="24").ask()
    ...
    def detect_token_in_headers(self, headers: Dict[str, str]) -> Optional[Tuple[str, str]]:
        """
        ...
        api_key_headers = ["X-API-Key", "X-Auth-Token", "X-Access-Token", "api_key", "apikey"]
    ...
    def extract_token_from_response(self, response_text: str) -> Optional[Dict[str, Any]]:
        """
        ...
        for token_id, token_data in self.tokens.items():
            ...
            if token_info.expires_at:
                ...
                if time_until_expiry.total_seconds() <= 0:
                    ...
                    status = f"Expires in {int(time_until_expiry.total_seconds() / 60)} min"
                else:
...
def demo_auth_manager():
    ...
    for token_info in tokens:
        print(f"   {token_info['environment']} - {token_info['type']} - {token_info['status']}")
