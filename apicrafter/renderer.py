...
class ResponseRenderer:
    ...
    def render_response(self, response: ResponseData, show_headers: bool = True) -> None:
        """
        ...
        status_text.append(f"Status: {response.status_code}", style=f"bold {status_color}")
        status_text.append(f" • Time: {response.response_time:.3f}s", style="dim")
        
        self.console.print(Panel(status_text, title="Request Info", border_style=status_color))
    ...
    def _render_json(self, json_data: Any) -> None:
        ...
        try:
            ...
            self.console.print(Panel(json_obj, title="Response Body (JSON)", border_style="green"))
        except Exception:
            # Fallback to regular text
            json_str = json.dumps(json_data, indent=2, ensure_ascii=False)
            syntax = Syntax(json_str, "json", theme="monokai", line_numbers=False)
            self.console.print(Panel(syntax, title="Response Body (JSON)", border_style="green"))
    ...
    def _render_html(self, html_content: str) -> None:
        ...
        try:
            ...
            syntax = Syntax(html_content, "html", theme="monokai", line_numbers=False, word_wrap=True)
            self.console.print(Panel(syntax, title="Response Body (HTML)", border_style="blue"))
        except Exception:
            self.console.print(Panel(html_content, title="Response Body (HTML)", border_style="blue"))
    ...
    def _render_xml(self, xml_content: str) -> None:
        ...
        try:
            ...
            self.console.print(Panel(syntax, title="Response Body (XML)", border_style="yellow"))
        except Exception:
    ...
    def _render_text(self, text_content: str, content_type: str) -> None:
        ...
        if lexer:
            try:
                syntax = Syntax(text_content, lexer, theme="monokai", line_numbers=False, word_wrap=True)
                self.console.print(Panel(syntax, title=f"Response Body ({lexer.upper()})", border_style="white"))
                return
            except Exception:
                pass
        
        # Fallback to plain text
        self.console.print(Panel(text_content, title="Response Body (Text)", border_style="white"))
    ...
    def render_history(self, history: List[HistoryEntry], limit: Optional[int] = None) -> None:
    ...
    def render_test_results(self, test_name: str, results: Dict[str, bool], all_passed: bool) -> None:
        """Render test results."""
        ...
        if results:
            ...
            for assertion, passed in results.items():
                ...
                table.add_row(assertion, f"[{result_color}]{result_text}[/{result_color}]")
