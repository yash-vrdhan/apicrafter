...
class InteractiveSession:
    ...
    def __init__(self, storage: Optional[StorageManager] = None, renderer: Optional[ResponseRenderer] = None):
    ...
    def run_schema_driven_request(self) -> None:
        ...
        try:
            ...
            if not schema:
                self.renderer.print_error("Failed to load schema. Falling back to manual mode.")
                self.run_manual_request()
                return
            
            # Show schema summary
            summary = self.schema_loader.get_schema_summary(schema)
            self.renderer.print_success(f"Loaded schema: {summary['title']} v{summary['version']}")
            ...
            endpoint_schema = self.schema_loader.get_endpoint_schema(schema, method, path)
            ...
            if endpoint_schema.auth_schema:
                auth_data = self.field_prompter.prompt_for_auth(endpoint_schema.auth_schema)
                if auth_data:
                    ...
                    if auth_data['type'] == 'bearer':
                        ...
                        auth_config = AuthConfig(
                            ...
                            credentials={'key_name': auth_data['key_name'], 'key_value': auth_data['key_value']},
            ...
            if endpoint_schema.headers:
                schema_headers = self.field_prompter.prompt_for_headers(endpoint_schema.headers)
                headers.update(schema_headers)
            
            # Query parameters
            if endpoint_schema.query_params:
                schema_params = self.field_prompter.prompt_for_query_params(endpoint_schema.query_params)
                params.update(schema_params)
            
            # Request body
            if endpoint_schema.body_schema:
                body_obj = self.field_prompter.prompt_for_body(endpoint_schema.body_schema)
                if body_obj:
                    json_data = body_obj
            ...
            if not validation_result.is_valid:
                ...
                for error in validation_result.errors:
                    self.renderer.console.print(f"  • {error.field}: {error.message}", style="red")
            ...
            self._show_request_summary(method, resolved_url, headers, params, None, json_data, environment)
            ...
            # Ask to save request
            self._offer_save_request(method, resolved_url, headers, params, None, json_data)
    ...
    def run_manual_request(self) -> None:
        ...
        try:
            ...
            # Confirm request
            self._show_request_summary(method, url, headers, params, body_data, json_data, environment, auth_config)
            ...
            try:
                ...
                # Ask to save request
                self._offer_save_request(method, url, headers, params, body_data, json_data)
    ...
    def _collect_headers(self) -> Dict[str, str]:
        ...
        if questionary.confirm("Add headers?", default=False).ask():
            while True:
                header_name = questionary.text("Header name (or press Enter to finish):").ask()
    ...
    def _collect_query_params(self) -> Dict[str, str]:
        ...
        if questionary.confirm("Add query parameters?", default=False).ask():
            while True:
                param_name = questionary.text("Parameter name (or press Enter to finish):").ask()
                ...
                if not questionary.confirm("Add another parameter?", default=False).ask():
                    break
    ...
    def _collect_body_data(self) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
        ...
        if body_type == "json":
            ...
            form_data = {}
            while True:
                field_name = questionary.text("Form field name (or press Enter to finish):").ask()
    ...
    def _show_request_summary(
        ...
        if auth_config and auth_config.auth_type.value != "none":
            ...
            self.renderer.console.print(f"Authentication: [bold]{auth_type_display}[/bold]")
    ...
    def _offer_save_request(
        ...
        if questionary.confirm("Save this request?", default=False).ask():
            ...
            if request_name:
                ...
                try:
                    self.storage.save_request(request_name, request_data, collection_name)
                    self.renderer.print_success(f"Request saved as '{request_name}' in collection '{collection_name}'")
                except Exception as e:
        ...
        for coll_name, collection in collections.items():
            if hasattr(collection, 'requests'):
                for req_name in collection.requests.keys():
                    display_name = f"{req_name} ({coll_name})" if coll_name != "default" else req_name
    ...
    def _delete_collection_interactive(self) -> None:
        ...
        choices = [Choice(name, name) for name in collections.keys() if name != "default"]
        
        if not choices:
            self.renderer.print_error("No collections available for deletion (cannot delete 'default')")
            return
        ...
        if questionary.confirm(f"Delete collection '{collection_name}' and all its requests?", default=False).ask():
