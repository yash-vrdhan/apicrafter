...
def send(
    ...
    header: Annotated[Optional[List[str]], typer.Option("--header", "-H", help="Headers in format 'Key: Value'")] = None,
    query: Annotated[Optional[List[str]], typer.Option("--query", "-q", help="Query params in format 'key=value'")] = None,
    body: Annotated[Optional[str], typer.Option("--body", "-b", help="Request body as string")] = None,
    json_data: Annotated[Optional[str], typer.Option("--json", "-j", help="Request body as JSON string")] = None,
    form: Annotated[Optional[List[str]], typer.Option("--form", "-f", help="Form data in format 'key=value'")] = None,
    raw: Annotated[Optional[str], typer.Option("--raw", help="Raw body data")] = None,
    auth: Annotated[Optional[str], typer.Option("--auth", "-a", help="Auth: 'bearer:TOKEN', 'basic:user:pass', 'apikey:name:value[:location]'")] = None,
    env: Annotated[str, typer.Option("--env", "-e", help="Environment to use")] = "default",
    no_headers: Annotated[bool, typer.Option("--no-headers", help="Don't show response headers")] = False,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Show detailed request info")] = False,
) -> None:
    ...
    try:
        ...
        if header:
            for h in header:
                if ":" in h:
                    ...
                    renderer.print_error(f"Invalid header format: {h}. Use 'Key: Value'")
                    raise typer.Exit(1)
        ...
        if auth:
            ...
            if auth_config:
                ...
                renderer.print_info("Valid formats: 'bearer:TOKEN', 'basic:user:pass', 'apikey:name:value[:location]'")
                raise typer.Exit(1)
        ...
        if verbose:
            ...
            if final_json:
                ...
                renderer.print_info(f"Body: {final_body[:200]}{'...' if len(str(final_body)) > 200 else ''}")
...
def save(
    ...
    body: Annotated[Optional[str], typer.Option("--body", "-b", help="Request body as string")] = None,
    json_data: Annotated[Optional[str], typer.Option("--json", "-j", help="Request body as JSON string")] = None,
    collection: Annotated[str, typer.Option("--collection", "-c", help="Collection name")] = "default",
) -> None:
...
def run(
    ...
    env: Annotated[str, typer.Option("--env", "-e", help="Environment to use")] = "default",
    no_headers: Annotated[bool, typer.Option("--no-headers", help="Don't show response headers")] = False,
    validate: Annotated[Optional[str], typer.Option("--validate", help="Validate against schema from URL or file path")] = None,
) -> None:
    """Run a saved request."""
    try:
        # Load request
        ...
        if not request_data:
            renderer.print_error(f"Request '{name}' not found in collection '{collection}'")
            raise typer.Exit(1)
...
def replay(
    ...
    try:
        history_data = storage.load_history(100)  # Get more entries to find the right one
...
def test(
    ...
    collection: Annotated[str, typer.Option("--collection", "-c", help="Collection name")] = "default",
    env: Annotated[str, typer.Option("--env", "-e", help="Environment to use")] = "default",
    tests_file: Annotated[Optional[str], typer.Option("--tests", "-t", help="Path to tests file")] = None,
) -> None:
...
def auth(
    ...
    credentials: Annotated[str, typer.Argument(help="Credentials (format depends on type)")],
    url: Annotated[str, typer.Option("--url", "-u", help="Test URL")] = "https://httpbin.org/bearer",
) -> None:
...
@app.command()
def curl(
    request_name: Annotated[str, typer.Argument(help="Name of saved request to convert to curl")],
    collection: Annotated[str, typer.Option("--collection", "-c", help="Collection name")] = "default",
    env: Annotated[str, typer.Option("--env", "-e", help="Environment to use")] = "default",
) -> None:
    ...
    try:
        ...
        if not request_data:
            renderer.print_error(f"Request '{request_name}' not found in collection '{collection}'")
            raise typer.Exit(1)
        ...
        try:
            ...
            renderer.print_info("💡 Tip: Copy the command above to use in your terminal")
        except:
            pass
...
def docs() -> None:
    ...
    examples = [
        ...
        ("With authentication:", "apicrafter send GET https://httpbin.org/bearer --auth 'bearer:your-token'"),
        ("Form data:", "apicrafter send POST https://httpbin.org/post --form 'name=John' --form 'age=30'"),
    ...
    renderer.console.print("  • Run 'apicrafter <command> --help' for command-specific help")
...
def tokens() -> None:
    ...
    try:
        ...
        for token_info in token_list:
            status_color = "green" if "Valid" in token_info["status"] else "red" if "Expired" in token_info["status"] else "yellow"
            table.add_row(
